...
# ===============================
import sys, subprocess, importlib, importlib.util, os, re, textwrap
from datetime import datetime

# ---------- 0) Dependency bootstrap (Colab-friendly, offline-safe) ----------
...
def ensure_runtime_dependencies():
    """Check for runtime packages and exit gracefully with guidance if missing."""
    required_mods = {
        "gradio": "UI",
    ...
    print(
        "[ERROR] Paket belum tersedia: " + ", ".join(missing) +
        "\n- Jika di Colab: set env TLC_SKIP_PIP=0 lalu jalankan ulang sel ini."
        "\n- Jika lokal: jalankan `pip install -r requirements.txt` atau set TLC_SKIP_PIP=0"
        " untuk mencoba auto-install."
    )
    sys.exit(1)
...
# Pastikan dependency tersedia (exit dengan pesan ramah jika tidak)
ensure_runtime_dependencies()

# Safe import setelah pengecekan
import gradio as gr
